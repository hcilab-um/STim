...
{
	#region PluginInfo
	[PluginInfo(Name = "FirstChip", Category = "Value", Version = "DynamicPins", Help = "Basic template with a dynamic amount of in- and outputs", Tags = "")]
	#endregion PluginInfo
	public class DynamicPinsValueFirstChipNode : IPluginEvaluate, IPartImportsSatisfiedNotification
	{
		#region fields & pins
		...
		public void Evaluate(int SpreadMax)
		{
			for (int i = 0; i < FInputs.CombineWith(FOutputs); i++) {
				...
				ISpread<double> results = new Spread<double>();
				for(int index = 0 ; index < inputSpread.SliceCount ; index++)
					results.Add(inputSpread[index] * 2);
				
				outputSpread.AssignFrom(results);
			}
		}
	}
}
